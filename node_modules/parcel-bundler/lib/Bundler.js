...
class Bundler extends EventEmitter {
    ...
    return _asyncToGenerator(function* () {
      ...
      if (_this.pending) {
        return new Promise(function (resolve, reject) {
          _this.once('buildEnd', function () {
            _this.bundle().then(resolve, reject);
          });
        });
      }

      let isInitialBundle = !_this.mainAsset;
      let startTime = Date.now();
      _this.pending = true;
      _this.errored = false;

      _this.logger.clear();
      _this.logger.status('⏳', 'Building...');
      ...
      try {
        ...
        if (isInitialBundle) {
          yield fs.mkdirp(_this.options.outDir);

          _this.mainAsset = yield _this.resolveAsset(_this.mainFile);
          _this.buildQueue.add(_this.mainAsset);
        }

        // Build the queued assets, and produce a bundle tree.
        let bundle = yield _this.buildQueuedAssets(isInitialBundle);

        let buildTime = Date.now() - startTime;
        let time = buildTime < 1000 ? `${buildTime}ms` : `${(buildTime / 1000).toFixed(2)}s`;
        _this.logger.status('✨', `Built in ${time}.`, 'green');
        ...
        if (!_this.watcher && _this.options.killWorkers) {
          _this.stop();
        }
      }
    })();
  ...
  start() {
    ...
    return _asyncToGenerator(function* () {
      ...
      _this2.options.extensions = Object.assign({}, _this2.parser.extensions);
      _this2.farm = WorkerFarm.getShared(_this2.options);

      if (_this2.options.watch) {
        _this2.watcher = new FSWatcher();
        _this2.watcher.on('change', _this2.onChange.bind(_this2));
      }

      if (_this2.options.hmr) {
        _this2.hmr = new HMRServer();
        _this2.options.hmrPort = yield _this2.hmr.start();
      }
    })();
  }
  ...
  buildQueuedAssets(isInitialBundle = false) {
    ...
    return _asyncToGenerator(function* () {
      ...
      while (_this3.buildQueue.size > 0) {
        let promises = [];
        ...
        try {
          for (var _iterator = _this3.buildQueue[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
            ...
            if (!isInitialBundle) {
              asset.invalidate();
              if (_this3.cache) {
                _this3.cache.invalidate(asset.name);
              }
            }

            promises.push(_this3.loadAsset(asset));
      ...
      if (_this3.hmr && !isInitialBundle) {
        _this3.hmr.emitUpdate([..._this3.findOrphanAssets(), ...loadedAssets]);
      }
      ...
      try {
        for (var _iterator2 = _this3.loadedAssets.values()[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
      ...
      let bundle = _this3.createBundleTree(_this3.mainAsset);
      _this3.bundleHashes = yield bundle.package(_this3, _this3.bundleHashes);

      // Unload any orphaned assets
      _this3.unloadOrphanedAssets();

      _this3.emit('bundled', bundle);
      return bundle;
    })();
  }
  ...
  resolveAsset(name, parent) {
    ...
    return _asyncToGenerator(function* () {
      var _ref = yield _this4.resolver.resolve(name, parent);

      let path = _ref.path,
          pkg = _ref.pkg;

      if (_this4.loadedAssets.has(path)) {
        return _this4.loadedAssets.get(path);
      }

      let asset = _this4.parser.getAsset(path, pkg, _this4.options);
      _this4.loadedAssets.set(path, asset);

      if (_this4.watcher) {
        _this4.watcher.add(path);
      }
  ...
  resolveDep(asset, dep) {
    ...
    return _asyncToGenerator(function* () {
      try {
        return yield _this5.resolveAsset(dep.name, asset.name);
      } catch (err) {
  ...
  loadAsset(asset) {
    ...
    return _asyncToGenerator(function* () {
      if (asset.processed) {
        _this6.buildQueue.delete(asset);
        return;
      }

      if (!_this6.errored) {
        _this6.logger.status('⏳', `Building ${asset.basename}...`);
      }
      ...
      let processed = _this6.cache && (yield _this6.cache.read(asset.name));
      if (!processed) {
        processed = yield _this6.farm.run(asset.name, asset.package, _this6.options);
        if (_this6.cache) {
          _this6.cache.write(asset.name, processed);
        }
      }
      ...
      if (_this6.delegate.getImplicitDependencies) {
        let implicitDeps = yield _this6.delegate.getImplicitDependencies(asset);
        if (implicitDeps) {
      ...
      yield Promise.all(dependencies.map((() => {
        var _ref2 = _asyncToGenerator(function* (dep) {
          let assetDep = yield _this6.resolveDep(asset, dep);
          if (dep.includedInParent) {
            // This dependency is already included in the parent's generated output,
            // so no need to load it. We map the name back to the parent asset so
            // that changing it triggers a recompile of the parent.
            _this6.loadedAssets.set(dep.name, asset);
          } else {
            asset.dependencies.set(dep.name, dep);
            asset.depAssets.set(dep.name, assetDep);
            yield _this6.loadAsset(assetDep);
          }
        });
  ...
  *findOrphanAssets() {
  ...
  onChange(path) {
    ...
    return _asyncToGenerator(function* () {
      let asset = _this7.loadedAssets.get(path);
      if (!asset) {
        return;
      }
      ...
      _this7.buildQueue.add(asset);
      clearTimeout(_this7.rebuildTimeout);

      _this7.rebuildTimeout = setTimeout(_asyncToGenerator(function* () {
        yield _this7.bundle();
      }), 100);
    })();
  }
